...
while q:
    ...
    for d in dirs:
        ...
        if next[0] <0 or next[0] >= len(text[0]) or next[1]<0 or next[1]>=len(text) or next[2] >9 or (di==(node[4]+2)%4 and node[4]!=-1) or node[2]<3 and next[2]==0 and node[4]!=-1:
            continue
        ...
        if next not in done or done[next]>node[3]+val:
            ...
            heappush(q,(node[3]+val,next[0],next[1],next[2],node[3]+val,di,node[-1]+[tuple(next[:2])]))
